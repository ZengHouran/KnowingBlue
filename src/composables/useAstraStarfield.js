...
export function useAstraStarfield(canvasRef, stageRef) {
  ...
  function initialize() {
    ...
    fallbackStars.value = Array.from({ length: Math.min(1600, quality.count) }, (_, i) => {
      ...
      return { x: 500 + stars[index] * 500, y: 500 - stars[index + 1] * 500, size: stars[index + 3] * .55 * 1.259712, opacity: Math.min(1, stars[index + 4] * 1.1664), color: stars[index + 6] > .72 ? '#ffc195' : '#bce4ff' };
    });
